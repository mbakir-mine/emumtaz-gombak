...
export type UpkkWrittenPaper = (typeof UPKK_WRITTEN_PAPERS)[number];
...
export const UPKK_WRITTEN_PAPER_MAX = 70;
export const UPKK_WRITTEN_TOTAL_MAX = UPKK_WRITTEN_PAPERS.length * UPKK_WRITTEN_PAPER_MAX;

export function upkkWrittenPaperForSubject(subjectCode: string | null | undefined): UpkkWrittenPaper | undefined {
  return UPKK_WRITTEN_PAPERS.find((paper) => paper.subjectCode === subjectCode);
}

export function isUpkkWrittenSubjectCode(subjectCode: string | null | undefined) {
  return Boolean(upkkWrittenPaperForSubject(subjectCode));
}
...
export function upkkPercentage(mark: number, fullMark = UPKK_WRITTEN_PAPER_MAX) {
  if (!Number.isFinite(mark) || !Number.isFinite(fullMark) || fullMark <= 0) return 0;
  return Number(((mark / fullMark) * 100).toFixed(2));
}
