...
export default function UpkkTrialManager({ schools, moduleAccesses, classes, students }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    const [marksResult, gradesResult] = await Promise.all([
      supabase.from('upkk_trial_paper_marks').select('*').eq('kod_sekolah', schoolCode).eq('tahun_akademik', year).eq('class_id', classId),
  ...
  }, [classId, hasAccess, schoolCode, year]);
  ...
  async function saveMarks() {
    ...
    const rows = papers.map((paper) => ({ kod_sekolah: schoolCode, tahun_akademik: year, class_id: classId, student_id: studentId, paper_code: paper.paperCode, markah: Number(draft[paper.key]) }));
    const { error } = await supabase.from('upkk_trial_paper_marks').upsert(rows, { onConflict: 'tahun_akademik,student_id,paper_code' });
    setMessage(error ? `Gagal menyimpan markah: ${error.message}` : `${papers.length} markah subjek berjaya disimpan.`);
    if (!error) await load();
    setPending(false);
  }
